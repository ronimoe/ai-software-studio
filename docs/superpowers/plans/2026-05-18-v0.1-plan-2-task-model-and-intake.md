...
| File | Purpose |
|---|---|
| `src-tauri/src/db/schema.sql` | Add `tasks`, `task_acceptance_criteria`, `task_constraints` tables |
...
### Task 1: Extend the schema with task tables

**Files:**
- Modify: `src-tauri/src/db/schema.sql`

- [ ] **Step 1: Append task tables**

Replace `src-tauri/src/db/schema.sql` entirely:
...
- [ ] **Step 2: Drop your dev DB to force a fresh schema**

Run: `rm -f "$HOME/Library/Application Support/AI Software Studio/app.db"`
Expected: file removed (or already absent). Drop-and-recreate is acceptable in v0.1 per the spec.
