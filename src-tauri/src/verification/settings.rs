...
impl SettingsRepository {
    ...
    pub async fn get_for_project(
        ...
        let rows: Vec<(String, String)> =
            sqlx::query_as("SELECT key, value FROM app_settings WHERE scope = ?")
                .bind(&scope)
    ...
    pub async fn set_for_project(
        ...
        for (key, value) in [
            ...
            sqlx::query("INSERT INTO app_settings (scope, key, value) VALUES (?, ?, ?)")
                .bind(&scope)
                .bind(key)
                .bind(value.clone().unwrap_or_default())
                .execute(&mut *tx)
                .await
...
fn scope(project_id: &str) -> String {
