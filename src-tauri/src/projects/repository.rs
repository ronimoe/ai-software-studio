...
impl ProjectRepository {
    ...
    pub async fn list(&self) -> Result<Vec<Project>, AppError> {
        ...
        Ok(rows
            .into_iter()
            .map(|(id, name, path, default_branch)| Project {
                id,
                name,
                path,
                default_branch,
    ...
    pub async fn get(&self, id: &str) -> Result<Project, AppError> {
        let row: Option<(String, String, String, String)> =
            sqlx::query_as("SELECT id, name, path, default_branch FROM projects WHERE id = ?")
                .bind(id)
                .fetch_optional(&self.db.pool)
                .await
                .map_err(|e| AppError::internal(format!("get project: {e}")))?;
        row.map(|(id, name, path, default_branch)| Project {
            id,
            name,
            path,
            default_branch,
